...
async function config(phase: string, { defaultConfig }: { defaultConfig: NextConfig }) {
  const completeSchema = await $RefParser.dereference("../profile-builder-schema/profile.schema.json");
  ...
  fs.writeFileSync("./src/_profile-builder-json-schema/profile.complete.schema.json", completeSchemaJSON);
