...
export const AssignmentCard = ({
  ...
  const assignment = form.watch(`controls.${controlIndex}.assignments.${assignmentIndex}`);

  const { confirm: confirmDelete, render: ConfirmDeleteComponent } = useConfirmModal({
    title: t("Are you sure?"),
    body: t("Are you sure you want to remove this assignment?"),
    onConfirm: () => onRemoveAssignment(),
  });

  const { open: openEditConditionsModal, render: EditConditionsModal } = useEditConditionsModal({
  ...
  const { open: openEditRailClassInformationModal, render: EditRailClassInformationModal } =
