...
impl Module {
    pub fn init(lua: Arc<&Lua>, runtime: &Lazy<Runtime>, restart_token: CancellationToken) -> Result<Table> {
        let exports = lua.create_table()?;
        let (socket_callback_tx, socket_callback_rx) = tokio::sync::watch::channel::<Option<Function>>(None);
        let direct_control_message_queue_arc: Arc<Mutex<VecDeque<String>>> = Arc::new(Mutex::new(VecDeque::new()));
        ...
        runtime.spawn(async move {
            loop {
                ...
                if task_cancel_token.is_cancelled() {
                    println!("[DC][LuaForwarder] Task Cancelled");
                    break;
                }
        ...
        let task_cancel_token = restart_token.child_token();
        let direct_control_message_queue_arc_clone: Arc<Mutex<VecDeque<String>>> = Arc::clone(&direct_control_message_queue_arc);
        runtime.spawn(async move {
            loop {
                /* don't reconnect if cancelled */
                if task_cancel_token.is_cancelled() {
                    println!("[DC][Websocket] Task Cancelled - Not reconnecting");
                    break;
                }
                        ...
                        loop {
                            tokio::select! {
                                _ = inner_cancel_token.cancelled() => {
                                    println!("[DC][Websocket] Task Cancelled - Closing Connection");
        ...
        runtime.spawn(async move {
            loop {
                /* don't reconnect if cancelled */
                if task_cancel_token.is_cancelled() {
                    println!("[SC][Outgoing] Task Cancelled - Not reconnecting");
                    break;
                }
                ...
                match connect_async(SYNC_CONTROL_WS_ADDR).await {
                    Ok((mut socket, _)) => {
                        ...
                        loop {
                            tokio::select! {
                                _ = inner_cancel_token.cancelled() => {
                                    println!("[SC][Websocket] Task Cancelled - Closing Connection");
                                    /* stop continuous read loop and close socket if cancelled */
                                    socket.close(None).await.unwrap();
                                    break;
                                },
        ...
        exports.set(
            ...
            lua.create_function(move |_, ()| {
                println!("[TSW5GamepadMod] Stopping instance");
        ...
        exports.set(
            ...
            lua.create_function(move |_, callback: Function| {
                /* send function ref to socket callback */
